	...
	////////Program Start
	  ...
	  function editCell(cell)
	  {
		...
		if(cell.has(":input").length==0){
			...
			{
				val=val.replace(/'/g,"-");			
			}else if(imemodeforsport=='field'  && cell.attr('id').substring(0,1)=='h' ){
				val=val.replace('X',"*");			
			}
	...
	function closeedit()
	{
		$('.M').each(function(i)
		{
			...
			//	alert(imemodeforsport);
				input=$(this).children();
				strv=input.val();
				if($(this).attr('id').substring(0,2)=='rc')
					{strv=strv.replace(/-/g,"'");}
				else if(imemodeforsport=='field'  && $(this).attr('id').substring(0,1)=='h'){
					strv=strv.replace('*','X');			
				}else if(imemodeforsport=='hj'  && $(this).attr('id').substring(0,1)=='h'){
					
					strv=strv.toUpperCase();
				}
				if($(this).attr('id').substring(0,4)=='note')
					{strv=strv.toUpperCase();}			    
				$(this).text(strv);
				input.remove();				
			}
